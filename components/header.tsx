    ...
    <header className="w-full bg-white/95 backdrop-blur-md shadow-lg border-b border-gray-100 fixed top-0 left-0 z-50">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 md:px-8 h-[80px]">
        ...
        <div className="flex items-center space-x-3 rtl:space-x-reverse">
          ...
          <Button 
            size="sm" 
            className="hidden md:flex bg-blue-600 hover:bg-blue-700 text-white shadow-md hover:shadow-lg transition-all duration-300"
          >
            Get Quote
          </Button>
            ...
            <nav className="flex flex-col space-y-2">
              {menuItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="text-gray-700 hover:text-blue-600 hover:bg-blue-50 transition-all duration-300 font-medium px-4 py-3 rounded-lg"
